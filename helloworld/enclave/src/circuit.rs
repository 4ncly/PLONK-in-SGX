...
impl Circuit for Yao{
    ...
    fn gadget(&mut self, composer: &mut dusk_plonk::constraint_system::TurboComposer) -> core::result::Result<(), dusk_plonk::error::Error> { 
        ...
        let bits_sub = composer.component_decomposition::<10>(sub);

        composer.assert_equal_constant(bits_sub[8],BlsScalar::from(self.c),None);



        // println!("{:?}",composer.witnesses[&(bits_sub[8])]==BlsScalar::from(1));
