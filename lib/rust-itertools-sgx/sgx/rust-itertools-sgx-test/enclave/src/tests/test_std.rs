...
pub fn unique_by() {
    ...
    itertools::assert_equal(ys.iter(), xs.iter().unique_by(|x| x[..2].to_string()));
}
...
pub fn unique() {
    ...
    let ys = [0, 1, 2, 3];
    itertools::assert_equal(ys.iter(), xs.iter().unique());
    let xs = [0, 1];
    let ys = [0, 1];
    itertools::assert_equal(ys.iter(), xs.iter().unique());
}
...
pub fn dedup() {
    ...
    let ys = [0, 1, 2, 1, 3];
    itertools::assert_equal(ys.iter(), xs.iter().dedup());
    let xs = [0, 0, 0, 0, 0];
    let ys = [0];
    itertools::assert_equal(ys.iter(), xs.iter().dedup());
...
pub fn tee() {
    ...
    itertools::assert_equal(t1, xs.iter().cloned());
    itertools::assert_equal(t2, xs.iter().cloned());

    let (t1, t2) = xs.iter().cloned().tee();
    itertools::assert_equal(t1.zip(t2), xs.iter().cloned().zip(xs.iter().cloned()));
}
...
pub fn merge_by() {
    ...
    itertools::assert_equal(results, expected.iter());
}

//#[test]
pub fn merge_by_btree() {
    ...
    itertools::assert_equal(results, expected.into_iter());
}
...
pub fn kmerge() {
    ...
    itertools::assert_equal(its.kmerge(), 0..10);
}
...
pub fn sorted_by() {
    ...
    itertools::assert_equal(sc, vec![1, 2, 3, 4]);

    let v = (0..5).sorted_by(|&a, &b| a.cmp(&b).reverse());
    itertools::assert_equal(v, vec![4, 3, 2, 1, 0]);
}
...
//#[test]
pub fn test_multipeek_peeking_next() {
...
//#[test]
pub fn pad_using() {
    itertools::assert_equal((0..0).pad_using(1, |_| 1), 1..2);

    let v: Vec<usize> = vec![0, 1, 2];
    let r = v.into_iter().pad_using(5, |n| n);
    itertools::assert_equal(r, vec![0, 1, 2, 3, 4]);

    let v: Vec<usize> = vec![0, 1, 2];
    let r = v.into_iter().pad_using(1, |_| panic!());
    itertools::assert_equal(r, vec![0, 1, 2]);
}
...
//            itertools::assert_equal(&mut subs[idx].1, text);
//        }
//    }
//
...
//        itertools::assert_equal(gr.into_iter().flat_map(|(_, sub)| sub), text.chars());
//    }
//}

//#[test]
pub fn group_by_lazy_2() {
    let data = vec![0, 1];
    let groups = data.iter().group_by(|k| *k);
    let gs = groups.into_iter().collect_vec();
    itertools::assert_equal(data.iter(), gs.into_iter().flat_map(|(_k, g)| g));

    let data = vec![0, 1, 1, 0, 0];
    let groups = data.iter().group_by(|k| *k);
    let mut gs = groups.into_iter().collect_vec();
    gs[1..].reverse();
    itertools::assert_equal(&[0, 0, 0, 1, 1], gs.into_iter().flat_map(|(_, g)| g));
    ...
    itertools::assert_equal(&mut groups[0], &[1, 1]);
    ...
    itertools::assert_equal(&mut groups[0], &[0, 0, 0]);
    itertools::assert_equal(&mut groups[1], &[1, 1]);
    itertools::assert_equal(&mut groups[2], &[3, 3]);
    ...
    for (i, group) in &grouper {
        match i {
            0 => itertools::assert_equal(group, &[0, 0, 0]),
            1 => itertools::assert_equal(group, &[1, 1, 0]),
            2 => itertools::assert_equal(group, &[0, 2, 2]),
            3 => itertools::assert_equal(group, &[3, 3]),
            _ => unreachable!(),
        }
    }
}
...
pub fn chunks() {
    ...
    for (i, chunk) in grouper.into_iter().enumerate() {
        match i {
            0 => itertools::assert_equal(chunk, &[0, 0, 0]),
            1 => itertools::assert_equal(chunk, &[1, 1, 0]),
            2 => itertools::assert_equal(chunk, &[0, 2, 2]),
            3 => itertools::assert_equal(chunk, &[3, 3]),
            _ => unreachable!(),
        }
    }
}
...
pub fn combinations() {
    ...
    itertools::assert_equal((0..0).tuple_combinations::<(_, _)>(), <Vec<_>>::new());
    itertools::assert_equal((0..1).tuple_combinations::<(_, _)>(), <Vec<_>>::new());
    itertools::assert_equal((0..2).tuple_combinations::<(_, _)>(), vec![(0, 1)]);

    itertools::assert_equal((0..0).combinations(2), <Vec<Vec<_>>>::new());
    itertools::assert_equal((0..1).combinations(1), vec![vec![0]]);
    itertools::assert_equal((0..2).combinations(1), vec![vec![0], vec![1]]);
    itertools::assert_equal((0..2).combinations(2), vec![vec![0, 1]]);
}

//#[test]
pub fn combinations_of_too_short() {
    for i in 1..10 {
...
//#[test]
pub fn combinations_zero() {
    itertools::assert_equal((1..3).combinations(0), vec![vec![]]);
}

//#[test]
pub fn diff_mismatch() {
    let a = vec![1, 2, 3, 4];
    let b = vec![1.0, 5.0, 3.0, 4.0];
    let b_map = b.into_iter().map(|f| f as i32);
    let diff = itertools::diff_with(a.iter(), b_map, |a, b| *a == b);

    assert!(match diff {
        Some(itertools::Diff::FirstMismatch(1, _, from_diff)) =>
...
pub fn diff_longer() {
    ...
    assert!(match diff {
        Some(itertools::Diff::Longer(_, remaining)) =>
...
pub fn diff_shorter() {
    ...
    assert!(match diff {
        Some(itertools::Diff::Shorter(len, _)) => len == 2,
        _ => false,
    });
}
...
pub fn while_some() {
    let ns = (1..10).map(|x| if x % 5 != 0 { Some(x) } else { None })
                    .while_some();
    itertools::assert_equal(ns, vec![1, 2, 3, 4]);
}
